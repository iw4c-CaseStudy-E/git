(function($) {
	...
	var cache	= {
		...
		aux		= {
			// triggered when we click a slice. If the slice is expanded,「expanded...拡張」
...
//							alert("33");
							//閉じたときに修正する部分を作成します。
							//閉じるときのLineHeightの値をここで設定します。
							var lh=20;
							
							//閉じるときの背景のエフェクトを設定します
							var opa=1;
...
//								$navNext.fadeOut();
						...
						$el.stop()
						   .animate( animParam, settings.animSpeed, settings.animEasing, function() {
								if( !expanded )//もし拡張してなかったら
									$el.find('.va-content').fadeIn( settings.contentAnimSpeed );
						   })
						   .find('.va-title')
						   .stop()
						   .animate({
							lineHeight	: lh + 'px',//	ここも確認するッス//（デフォルトはlineHeight	: cache.sliceH + 'px'）ここは開いた時のLineHeightの箇所です。
						   },settings.animSpeed, settings.animEasing);	
						//settings.animSpeed, settings.animEasing, 
						//--------------------------------------------------------------------
//						$(".va-content").animate( animParam, settings.animSpeed, settings.animEasing, function() {
//						   })
//						   .animate({
//						   },{ duration: settings.animSpeed, easing:settings.animEasing , queue: false } );	
						//---------------------------------------------------------------------------
						...
						$others.each(function(i){
							...
							$other.stop()	//クリックしたときのほかの部分
								  ...
								  .stop()
								  .animate({
								//開いた時にほかのsliceのLineHeightです。
										lineHeight	: 20 + 'px'//othersHeight+'px'
								  }, settings.animSpeed, settings.animEasing )
								  .end()
								  .find('.va-content')
								  .hide();
					...
					if( !settings.savePositions || cache.idx_expanded === -1 )
						...
						if( i === $nextSlice.index() ) {
							...
							$slice.stop()
								  ...
								  .stop()
								  .animate({
										lineHeight	: 20 + 'px'//テストッス(cache.sliceH + 'px')
								  }, settings.animSpeed, settings.animEasing );
						}
						// if the slice is the one opened, lets close it
							...
							$slice.stop()
								  ...
								  .stop()
								  .animate({//
										lineHeight	: othersHeight + 'px'//(othersHeight+'px')
								  }, settings.animSpeed, settings.animEasing )
								  .end()
								  .find('.va-content')
								  .hide();		  
			...
			init 		: function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						.children('.va-title')
						.css( 'line-height', 20 + 'px' );	//ここで初期状態のタイトルのlineHeightを設定してるッス！デフォは('line-height', cache.sliceH + 'px')
